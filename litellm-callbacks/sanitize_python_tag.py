...
Trailing-garbage shapes stripped:
    ...
    5. Naked turn-trailer:      `turn{0}{search}{0}` (no PUA prefix)
    6. Naked tool-pair trailer: `{search}{0}` / `{youtube}{0}` (turn 누락 변형)
    7. Harmony channel scaffold: `<|channel>thought<|channel|>` (gemma-4 leaks
...
_CJK_TARGET_TRIGGER_RE = re.compile(
    ...
    r"\b(?:translate|render|write|reply|respond|answer)\s+(?:to|in)\s+(?:chinese|japanese|cantonese|mandarin|kanji)\b|"
    r"\bin\s+(?:chinese|japanese|kanji|hanzi)\b|"
    r"中文|日本語|日语|繁體|繁体|簡體|简体|広東語|粤语",
    re.IGNORECASE,
)
...
def _user_requested_cjk_target(data: Any) -> bool:
    ...
    for m in msgs:
        ...
        if m.get("role") not in ("user", "system"):
            continue
        c = m.get("content")
        if isinstance(c, str) and _CJK_TARGET_TRIGGER_RE.search(c):
            return True
        if isinstance(c, list):
            for part in c:
                t = (part or {}).get("text") if isinstance(part, dict) else None
                if isinstance(t, str) and _CJK_TARGET_TRIGGER_RE.search(t):
                    return True
    return False
...
# (a chat-template strips wrong, or the model emits them mid-text),
...
# a PUA prefix. Variants seen in the wild:
#   turn{0}{search}{0}
#   turn{0}{youtube}{0}
...
# gemma-4 (and other harmony-trained checkpoints) leak thinking-channel
# scaffolding into content on multi-turn tool responses: the model emits the
# `<|channel|>thought ... <|channel|>` markers that should delimit a reasoning
# channel, but no --reasoning-parser is configured for gemma (multimodal
# constraint), so they surface verbatim before the real answer. The gemma
# tokenizer treats them as ordinary text (not atomic special tokens), so they
# usually arrive mangled — observed shape: `<|channel>thought\n<channel|>{answer}`.
# These control tokens never belong in user-facing content; strip them wherever
# they appear. Each alternative requires at least one `|` to avoid eating
# legitimate `<word>`-style text.
_HARMONY_NAMES = "channel|message|start|end|return|constrain"
_HARMONY_TOK = (
    r"<(?:\|(?:" + _HARMONY_NAMES + r")\|?|(?:" + _HARMONY_NAMES + r")\|)>"
)
# channel header token + its optional role word (thought/analysis/final/...)
HARMONY_SCAFFOLD_RE = re.compile(
    _HARMONY_TOK + r"\s*(?:thought|analysis|final|commentary|assistant|tool)?\s*"
)
HARMONY_TOK_RE = re.compile(_HARMONY_TOK)


def _strip_harmony(text: str) -> str:
    """Remove leaked harmony channel/message scaffolding (gemma-4 multi-turn
    tool responses). Returns text unchanged when no harmony token is present."""
    if "|>" not in text and "<|" not in text:
        return text
    if not HARMONY_TOK_RE.search(text):
        return text
    out = HARMONY_SCAFFOLD_RE.sub("", text)
    out = HARMONY_TOK_RE.sub("", out)
...
class PythonTagSanitizer(CustomLogger):
    async def async_post_call_success_hook(self, data, user_api_key_dict, response):
        try:
            cjk_target = _user_requested_cjk_target(data)
            ...
            for choice in choices:
                ...
                # 1a. harmony channel scaffold (gemma-4 multi-turn tool leak —
                #     `<|channel>thought<|channel|>` before the real answer).
                harmonized = _strip_harmony(stripped)
                if harmonized != stripped:
                    log.warning(
                        "python-tag: stripped harmony channel scaffold (model=%s)",
                        getattr(response, "model", "?"),
                    )
                stripped = harmonized
    ...
    async def async_post_call_streaming_iterator_hook(
        ...
        # 사용자가 *CJK target language* 명시 → han→kor 자동 변환 skip.
        cjk_skip = _user_requested_cjk_target(request_data)
        buffer = ""
        held: list[Any] = []
        passthrough = False
        pua_seen = False
        ...
        try:
            async for chunk in response:
                ...
                if isinstance(text, str) and text:
                    if turn_hold:
                        ...
                        if TURN_GLOBAL_RE.match(turn_buf):
                            ...
                            for fc in flushed:
                                if passthrough:
                                    yield _filter_cjk(fc, skip=cjk_skip)
                                else:
                                    held.append(fc)
                            continue
                        ...
                        if pre:
                            ...
                            delta.content = pre
                            if passthrough:
                                yield _filter_cjk(chunk, skip=cjk_skip)
                            else:
                                held.append(chunk)
                                buffer += pre
                            delta.content = post
                ...
                if TAG not in buffer and (len(buffer) >= 64 or finish):
                    ...
                    # flush and switch to direct streaming. The harmony channel
                    # scaffold (gemma-4) always sits at the very start, so it is
                    # fully contained in the buffered prefix: strip it, collapse
                    # the cleaned text into the first held chunk, null the rest.
                    # Also lstrip leading whitespace (thinking models leave \n\n at
                    # the start of the visible answer after reasoning is split off).
                    cleaned_buf = _strip_harmony(buffer).lstrip()
                    if cleaned_buf != buffer and held:
                        for hc in held:
                            if getattr(hc, "choices", None):
                                d = getattr(hc.choices[0], "delta", None)
                                if d is not None and getattr(d, "content", None) is not None:
                                    d.content = None
                        first = held[0]
                        if getattr(first, "choices", None):
                            d = getattr(first.choices[0], "delta", None)
                            if d is not None:
                                d.content = cleaned_buf or None
                        log.warning(
                            "python-tag: stripped harmony channel scaffold in stream (model=%s)",
                            getattr(chunk, "model", "?"),
                        )
                    for hc in held:
                        yield _filter_cjk(hc, skip=cjk_skip)
                    held = []
                    passthrough = True
                ...
                if TAG in buffer:
                    ...
                    if payloads:
                        ...
                        if new_calls:
                            ...
                            yield _filter_cjk(last, skip=cjk_skip)
                            return
