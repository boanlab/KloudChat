...
# litellm-config.yaml 자체는 gitignored — 사용자 배포 한정. 없으면 example 에서
...
# vLLM max_model_len 디스커버리 실패 시 emit 용 ctx 폴백. 정상 시엔 deployment 마다
# `/v1/models` 의 max_model_len 을 직접 쓴다.
CTX_FALLBACK=32768
...
# 시 OR 로 자동 우회. LibreChat dropdown 엔 미노출(gen-librechat 가 안 emit). 로컬
# primary 가 배포된 경우(url 비어있지 않음)만 emit.
emit_or_fallback() {
...
# LiteLLM router 의 enable_pre_call_checks 는 token count 시 messages 만 보고
# tools schema + chat-template wrapper 는 제외한다. tool-heavy agent 류는 그
# overhead 가 ~1.5K~4K 이라 작은 ctx (16K) 노드에서 router 가 OK 라고 본
...
__declared_max_input_tokens() {
  ...
  # 매우 작은 ctx 의 floor — declared 가 너무 작아지면 의미 없으므로 1024 미만이면
  # ctx 의 절반으로 fallback.
...
# 출력을 dedup — vllm_union_node_models 자체는 이미 dedup 하지만, fallback 경로는
# raw csv 를 그대로 iterate 하므로 여기서 한 번 더 차단해 AUTOGEN 중복 deployment
# 를 막는다.
...
emit_vllm_chat() {
  ...
  # 폴백 chain: (1) vLLM /v1/models 의 max_model_len — scheduler 가 노드별로 다른 값
  # 박을 수 있으므로 deployment 마다 따로 조회한다. (2) 실패 시 CTX_FALLBACK
  ...
  while IFS= read -r url; do
    ...
    echo "    model_info:"
    # vLLM 은 --enable-auto-tool-choice + tool-call-parser 로 native function calling
    # 을 지원한다. 이 플래그를 노출해야 LibreChat agent 가 ReAct(action/action_input)
    # 텍스트 fallback 대신 구조화된 tool_calls 로 도구를 실제 실행한다.
    echo "      supports_function_calling: true"
    echo "      supports_tool_choice: true"
    # 두 field 분리 — declared 는 router 라우팅 결정 (보수적, tools schema buffer 확보),
    # actual 은 truncate_to_ctx 의 trim 한도 (deployment 의 실제 capacity 활용).
    # 노드별로 다른 ctx 면 router 가 입력 토큰 수 기반으로 deployment 자동 선택.
...
# 로컬 vLLM 이 없어도 에이전트(Super Agent/스튜디오/Deep Research)의 두뇌가 풀린다.
...
# 호출이 LiteLLM 에 별도 logged 되어 double-count 방지).
emit_super_agent() {
  ...
  # shim 의 CHAT_MODEL=gemma-4-26b 가 큰 ctx 노드로 LB 되므로 그 ctx 를 그대로 노출한다.
  # 이게 없으면 LibreChat 가 모델명으로 작은 기본값(~38K)을 추정해 긴 입력
  # (대용량 파일 첨부 등) 을 보내기도 전에 프루닝으로 거부한다.
...
SECTION=$(
  ...
  # 챗/리서치 두뇌: 로컬 vLLM 있으면 로컬, 없고 OR 키 있으면 OR 동일모델로 직결.
  ...
  # --- 그 외: 로컬 vLLM → OR 동일모델 폴백 twin (router_settings.fallbacks 가 참조, UI 비노출) ---
