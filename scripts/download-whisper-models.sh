...
if [[ "$(detect_arch)" == amd64 ]]; then
  ...
  docker compose -f "$COMPOSE_FILE" ps -q whisper 2>/dev/null | grep -q . \
    || { err "whisper 컨테이너 미기동 — 먼저 ./scripts/install-whisper.sh"; exit 1; }
  if [[ $# -eq 0 ]]; then
    if has_nvidia_gpu; then set -- large-v3; else set -- small; fi
  fi
  expanded=(); for a in "$@"; do
    [[ "$a" == all ]] && expanded+=(tiny base small medium large-v3 turbo) || expanded+=("$a")
  done; set -- "${expanded[@]}"
  for name in "$@"; do
    info "pull $name (container)"
    # device=cpu/int8 → GPU 없이 다운로드+검증만. download_root 는 app.py 와 동일 경로.
    docker compose -f "$COMPOSE_FILE" exec -T whisper python3 - "$name" <<'PY'
import sys
...
getent passwd "$USR" &>/dev/null \
  || { err "user '$USR' 없음 — install-whisper.sh 가 만든다."; exit 1; }
