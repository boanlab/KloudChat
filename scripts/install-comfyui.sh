...
# 정책: ComfyUI 는 PRO5000 / PRO6000 / GB10 클래스에만 설치. RTX4090 (24GB) /
# RTX5090 (32GB) 는 flux fullset + LLM 동거 시 OOM 가능성 커서 차단. PRO5000
# (48GB) 는 ComfyUI 전용으로 운영하면 fit 하므로 허용 (단 같은 노드에 vLLM
# gemma-4-26b + KV + bge + Whisper 까지 다 올리면 빠듯하니 운영자가 NODES_VLLM 등을
# 신중히 분리할 책임).
...
# 정책 게이트(위 detect_gpu_class)는 공통 적용. amd64 는 docker-compose.media.yml 로
# ComfyUI 를 띄운다 — systemd(useradd/venv/unit) 경로는 건너뛴다.
if [[ "$(detect_arch)" == amd64 ]]; then
  ...
  # 모델/출력/Manager 상태는 호스트 볼륨. /var/lib 하위는 보통 root 만 mkdir 가능.
  ...
  env "${cenv[@]}" docker compose -f "$COMPOSE_FILE" up -d comfyui
...
# Blackwell 카드는 torch 2.9 계열로 픽스 — SM 12.0 dispatch 안정성 확보.
case "$(detect_gpu_class)" in
...
if (( SKIP_PIP == 0 )); then
  ...
  install_node() {
    ...
    # `[[ ... ]] && cmd` 는 [[ ]] 가 false 일 때 함수의 마지막 statement 가 exit 1 이 되어
    # set -e 가 호출자를 죽인다 (reqs="" 인 GGUF 호출이 정확히 그 경우). if/then 으로 명시.
  ...
  # (EmptyLTXVLatentVideo/LTXVConditioning/LTXVScheduler)는 최신 ComfyUI 코어에 내장.
...
# dynamic VRAM 의 본래 목적은 작은 VRAM 카드 (RTX4090 24GB 등) 에서 flux 같은 큰
# 모델을 swap 으로 돌리는 것. 이 프로젝트의 정책상 ComfyUI 가 도는 노드는
# PRO5000 (48GB) / PRO6000 (90GB usable) / GB10 (96GB usable) 세 종류뿐이고
# 셋 다 flux fullset (~22 GB) 이 단독으로 fit 한다. 세 클래스 모두 dynamic
# VRAM 의 이득이 사실상 0 (또는 음수):
...
#                      마진이 작아 dynamic 의 메모리 회계 noise 가 OOM 으로
#                      직결될 수 있어 결정적 로딩이 더 안전.
# 결론: ComfyUI 가 도는 모든 노드에서 --disable-dynamic-vram 을 강제. estimate-
# based 로딩이 fallback 으로 동작. (RTX4090/RTX5090 에서 --force 로 강제 설치한
# 경우엔 작은 VRAM 이라 dynamic 의 swap 이 도움될 수도 있지만, --force 자체가
# 정책 외 사용이라 동일 플래그를 유지한다.)
...
# 같은 호스트에 comfyui-shim 컨테이너가 있으면 재기동해서 /object_info variant
# 캐시 리프레시. compose 가 다른 호스트면 자동 skip — 그쪽에선 사용자가
