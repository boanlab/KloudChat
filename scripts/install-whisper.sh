...
# float16: 모든 NVIDIA GPU(Maxwell+) 지원하는 whisper 표준. int8_float16 은 일부
...
# amd64 CUDA 휠은 GPU 추론(float16) 지원 → docker-compose.media.yml 로 띄운다. arm64
# (aarch64 ct2 CPU-only int8)는 systemd 경로를 유지한다.
if [[ "$(detect_arch)" == amd64 ]]; then
  ...
  env "${wenv[@]}" docker compose -f "$COMPOSE_FILE" up -d whisper
...
if has_nvidia_gpu; then
  ...
  # compute type 을 probe 해서 폴백한다 (아래 "compute type probe").
else
...
# 프로젝트 repo 의 app.py 를 설치 루트에 복사한다 (버전 동기 목적).
...
# compute type probe — 설정된 COMPUTE_TYPE 이 ct2 가 실제 지원하는 타입인지 검증.
# GPU 가 있어도 ct2(특히 aarch64 CPU-only 휠)가 CUDA 를 못 잡으면 device 는 cpu 로
# 떨어지고, cpu 는 float16/int8_float16 미지원 → 런타임 500. 미지원이면 안전한 int8 로
# 폴백한다(cpu·gpu 양쪽에서 유효). probe 가 실패하면(예외) 원래 값 유지.
