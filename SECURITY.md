...
kchat is pre-1.0. Fixes land on `main` and in the next tagged release; there
...
- Findings in [`kloudchat-backend`][backend] — report them there. Model
...
- **Terminate TLS in front of kchat** and set `KCHAT_COOKIE_SECURE=true`. The
  refresh token is an httpOnly cookie; over plain HTTP it is readable on the
  wire.
- **Keep LiteLLM off the public network.** kchat's `/llm` route is the only
