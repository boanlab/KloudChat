...
│  │                                             │        prompt_id → 노드 고정)
│  │                                             │   │
│  │  whisper-shim (OpenAI /v1/audio 라우터)     │ ──┼─→ WHISPER_URLS (csv)
│  │                                             │       (inflight 카운터 + ollama VRAM
│  │                                             │        기준으로 노드 선택)
│  └─────────────────────────────────────────────┘
└─────────────────────────────────────────────────────┘
```
...
### Whisper (음성 인식 폴백)
faster-whisper + FastAPI 의 OpenAI-compatible `/v1/audio/transcriptions` endpoint. 자막 없는 YouTube 영상이 들어오면 `mcp/youtube.py` 가 `yt-dlp` 로 audio 추출 → **whisper-shim** 으로 POST → shim 이 라우팅한 backend 가 텍스트 반환. Ollama / ComfyUI 와 같은 패턴으로 **호스트 native** (venv + systemd) — `scripts/install-whisper.sh` 가 `/opt/whisper/{venv,app.py}` + `/var/lib/whisper` (HF 모델 캐시) 만들고 `:9000` 에 바인딩. `WHISPER_URLS` 이 비어있거나 backend 미가용이면 youtube MCP 가 LiteLLM 의 `whisper-1` (OR 경유) 로 자동 폴백.
...
`WHISPER_URLS` 에 여러 backend 를 넣으면 `whisper-shim` (Dockerfile.whisper-shim, python:3.11-slim) 이 라우터로 동작합니다. 매 요청마다 ① 캐시된 `/health` (10s TTL) 로 reachable 노드 필터 → ② shim 내부 in-flight 카운터로 가장 한가한 노드 선택 → ③ tie-break 으로 같은 호스트 ollama (`:11434/api/ps`) 의 VRAM 점유량 비교. ComfyUI 라우팅과 달리 whisper 는 매 호출이 self-contained 한 단일 HTTP 라운드트립이라 prompt_id 같은 stickiness 가 필요 없고, 모든 backend 가 동일 모델을 서빙하므로 alias/variant 디스커버리도 생략. LibreChat 의 `WHISPER_URL` 은 항상 `http://whisper-shim:9000` 을 가리키고, shim 만 `WHISPER_URLS` 의 backend 들을 알면 됩니다.
...
내부 전용: mongodb:27017, meilisearch:7700, vectordb:5432,
          ...
          whisper-shim:9000, deep-research:8081
호스트:   ollama:11434  (LiteLLM router 가 직접 호출),
          comfyui:8188  (comfyui-shim upstream),
...
에이전트 명명: `Text` / `Text + Code` / `Text + Image + Code` 3가지 — 능력 요약 (보유 빌트인 툴). 자세한 매핑은 [모델 설정](models.md#모델-선택-메커니즘).

`generate_image` 는 ollama 에이전트만 부착 — comfyui-shim → ComfyUI (flux/qwen-image, 노드별 VRAM-aware 라우팅으로 큰 LLM 과 ComfyUI 가 한 노드에 몰리지 않도록 분산). 외부 provider (openai / anthropic / google) 에이전트는 image tool 비부착 (`manage.sh` 의 `builtinFor` 가 외부 provider 에선 자동 drop).
