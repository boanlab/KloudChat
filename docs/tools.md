...
- **이미지 백엔드** — `image-generation` 툴이 `model` arg 에 따라 ComfyUI 또는 외부 image 모델로 분기.
...
| `image-generation` | `comfyui-shim` (→ ComfyUI 또는 외부 image 모델) |
...
`image-generation` 은 텍스트→이미지 또는 이미지 편집. `model` arg 에 따라 백엔드가 분기 — [이미지 백엔드](#이미지-백엔드) 참고.
...
| `math` | `uvx mcp-sympy` |
| `calculator` | `uvx mcp-server-math` |
| `semantic_scholar` | `uvx semantic-scholar-mcp` |
| `arxiv` | `uvx arxiv-mcp-server` |
| `litellm_usage` | `uv run --script /app/mcp/litellm_usage.py` |
...
**`calculator`** — 16 tools. 사칙연산 + power / sqrt / sum / product / compare. math 의 작은 모델용 대체.

**`semantic_scholar`** — 33 tools. 학술 논문 검색 / 인용 그래프 / 저자 / 추천. 키 없이 동작하지만 공용 IP 1 req/sec 제한 — multi-user 부하 클 때는 [API 키 발급](https://www.semanticscholar.org/product/api) 후 `env.SEMANTIC_SCHOLAR_API_KEY` 주입.

**`arxiv`** — 8 tools. `search_papers`, `download_paper`, `read_paper`, `list_papers`, `semantic_search` (로컬 컬렉션), `citation_graph`, `watch_topic`, `check_alerts`. arxiv.org public API 직결. 다운로드 캐시는 컨테이너 내부 `~/.arxiv-mcp-server/papers` (재시작 시 휘발 — 영구 보관 필요하면 host bind + `--storage-path` 인자).

**`litellm_usage`** — 2 tools. `my_usage(days)`, `budget_status()`. 사용자 본인의 LiteLLM virtual key 사용량/예산 조회. `mcp/litellm_usage.py` 가 PEP-723 inline deps 로 mcp + httpx 자동 설치.

### 큰/작은 모델 분리 정책

`math` / `semantic_scholar` / `arxiv` 는 schema 가 크거나 학술 도메인 특화라서 작은 모델 (≤10B 활성 param) 한테 호출 emit 실패가 잦습니다. `SMALL_MODELS` Set 에 명시된 모델만 `calculator` 만 부착하고 scholar/arxiv 제외:
...
`scholar` 와 `arxiv` 는 역할이 다릅니다 — scholar 는 메타데이터/인용 그래프/저자/추천 (검색·발견), arxiv 는 논문 ID 가 정해진 뒤 전문을 받아서 읽기 (분석). 함께 부착하면 "scholar 로 찾고 → arxiv 로 읽기" 흐름이 한 세션에서 가능.

### `litellm_usage` 의 startup: false
...
litellm_usage:
  ...
  startup: false
  command: uv
  args: [run, --script, /app/mcp/litellm_usage.py]
  env:
...
MCP 측은 `COMMON` (= `fetch_url`, `time`, `litellm_usage`) 가 전 에이전트 공통, 거기에 모델 크기별로 `calculator` 또는 `math + scholar + arxiv` 가 추가됩니다.
...
### 작은 ollama 모델 제외 (TOOL_EXCLUDE)

`qwen3.5:9b`, `llama3.1:8b` 는 `execute_code` + `image-generation` 을 빼고 부착합니다. 6툴 schema 동시 노출 시 호출 emit 실패가 end-to-end 매트릭스에서 확인됨 — 작은 모델은 도구 선택 부담이 큼.
...
native 자사 image 모델이 있는 provider 만 `image-generation` 부착. Anthropic 은 자사 image API 가 없어서 툴 자체 제외 (이름도 `Text + Code` 로 빠짐).
...
1. `librechat.yaml` 의 `mcpServers` 에 stdio 항목 추가:
   ```yaml
   my_server:
     type: stdio
     command: uvx
     args: [<package-name>]
   ```
2. `manage.sh` 의 `MCP_COMMON` (전 에이전트) 또는 모델 크기별 분기 (`MCP_*_BIG` / `MCP_*_SMALL`) 에 `sys__all__sys_mcp_my_server` 추가.
3. `./scripts/manage.sh agent sync` — 기존 유저 전체에 도구 upsert.
4. `docker compose restart librechat` — yaml 변경 적용.

도구 개수가 많은 서버 (≥30) 는 작은 모델용 별도 라이트 버전을 두는 게 안전합니다 (math/calculator 분리 패턴 참고).

### Built-in 도구 정책 변경

특정 모델에서 빌트인 도구를 빼고 싶으면 `manage.sh` 의 `TOOL_EXCLUDE` 에 추가 후 `agent sync`. `image-generation` 만 빼려는 경우, 외부 LLM provider 면 `EXT_IMAGE_FOR_PROVIDER` 에서 매핑 제거하면 자동으로 빠짐.

### 새 ComfyUI 이미지 alias 추가

같은 (kind, filename) 셋을 4곳에 동기 등록해야 디스커버리 + 라우팅이 일치합니다:

1. `comfyui-shim/workflows/<alias>-txt2img.json` — ComfyUI 워크플로 템플릿
2. `comfyui-shim/app.py` 의 `MODEL_ALIASES` + `COMFYUI_ALIAS_FILES`
3. `rag-patches/patch_librechat_sd_model.js` 의 `model` enum
4. `scripts/lib.sh` 의 `__comfyui_node_models` 파일명 매핑

### 새 외부 image 모델 추가

1. `litellm-config.yaml` 의 정적 섹션 (autogen 영역 밖) 에 `model_name: image-<alias>` 등록
2. `docker-compose.yml` 의 `comfyui-shim.environment.OR_IMAGE_MODELS` 에 `<alias>=image-<alias>` 추가
3. `scripts/manage.sh` 의 `EXT_IMAGE_FOR_PROVIDER` 에 provider→alias 매핑 추가
4. `docker compose up -d litellm comfyui-shim librechat`
...
| `litellm_usage` 빈 이메일 | yaml `startup: false` 누락 |
| image-generation 항상 ComfyUI | `OR_IMAGE_MODELS` 매핑 누락 |
...
**`litellm_usage` 빈 이메일** — yaml 에 `startup: false` 빠짐. app-level init 단계에 user 컨텍스트가 없어서 `{{LIBRECHAT_USER_EMAIL}}` placeholder 치환이 실패.

**image-generation 항상 ComfyUI** — `comfyui-shim.environment.OR_IMAGE_MODELS` env 매핑이 비어있거나 `model` arg 가 매핑에 없는 값. shim 이 매핑 못 찾으면 ComfyUI 로 폴백.
