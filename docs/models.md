...
`generate_image` 은 **ollama 에이전트 전용** — `model` arg 는 `flux-schnell` / `flux-dev` / `qwen-image` / `qwen-image-edit` 중 하나로 ComfyUI 로 라우팅. 외부 provider (openai/anthropic/google) 에이전트는 image tool 자체 비부착.
