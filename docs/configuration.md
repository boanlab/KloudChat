...
kchat is configured in two places, and the split matters:
...
| --- | --- | --- | --- |
| `BACKEND_BASE_URL` | `BACKEND_BASE_URL` | — | The kloudchat-backend gateway. Setting this alone derives all six feature endpoints by appending paths. |
...
| `KCHAT_ADMIN_PASSWORD` | `BOOTSTRAP_ADMIN_PASSWORD` | — | Change the example value before exposing the instance. |
...
| `KCHAT_API_URL` | `KCHAT_API_URL` | `http://kchat-api:8100` | nginx upstream, resolved at run time so the API can move hosts without an image rebuild. |
...
| `LITELLM_BUDGET_HEADROOM` | `0.2` | How far above the kchat allowance the proxy-side budget sits. A backstop that sits exactly on the limit fires first, blocking someone with a number no screen shows them. |
