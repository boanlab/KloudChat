...
- HTML Grid와 변환 Markdown의 빈 구조 행을 보존함. 구조화된 표가 있는 경우에만 빈 행을 구분선으로 취급하지 않도록 함.
- DOCX·PDF·HWPX 공통 전처리에서 구조 행을 유지하되, 일반 Markdown 빈 행 정리와 전부 빈 표의 생략 동작을 유지함. 기존 문서 생성 라이브러리는 변경하지 않음.

## 자동 검증

- 새 테스트 26개 통과함. 합성 SQLite에서 실제 FastAPI PATCH·GET·버전 기록·내보내기 경로를 검증함. 인증 의존성만 합성 사용자로 대체함.
...
- 보고서·richtext·디자인·HWPX·다중 처리 focused 564개 통과함.
- main 기반 전체 offline API 2,436개 통과, 1개 skip, 기존 warning 11개임. 별도 하네스에서 실제 socket·HTTP 전송을 차단했고 socket 시도 82개 차단, 실제 HTTP 전송 0개임.
