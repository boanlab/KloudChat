...
3. (옵션) `qwen3.5:9b` 같은 가벼운 모델 — Claude Code 의 Haiku 슬롯에 매핑
...
export ANTHROPIC_DEFAULT_SONNET_MODEL="ollama/qwen3.6:35b"
...
| `ANTHROPIC_DEFAULT_SONNET_MODEL` | `ollama/qwen3.6:35b` |
| `ANTHROPIC_DEFAULT_HAIKU_MODEL` | `ollama/qwen3.5:9b` |

`ANTHROPIC_MODEL` 은 명시 호출 시. OPUS 슬롯은 가장 어려운 작업 — OR 키 있으면 `anthropic/claude-opus-4.7` 로 대체 가능. SONNET 은 균형형 — VRAM 여유 있으면 `ollama/qwen3-coder-next:q8_0` 로 업그레이드. HAIKU 는 가벼운 작업.
