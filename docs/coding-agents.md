...
- `qwen3-coder-next:q4_K_M` (~51 GB VRAM, 빠름) vs `q8_0` (~84 GB VRAM, 고품질) 선택 가능
- 같은 LiteLLM 게이트웨이를 LibreChat 도 공유 — 키·예산·로깅 통일

## 사전 준비

1. KloudChat 띄워져 있고 LiteLLM healthy (`http://localhost:8000/health/liveliness` → 200)
2. Ollama 노드에 `qwen3-coder-next:q4_K_M` 또는 `q8_0` pull 됨 — `ollama pull qwen3-coder-next:q8_0`
3. (옵션) `gemma4:26b` 도 pull — Claude Code 의 Haiku 슬롯에 매핑
...
라우팅: Claude Code 가 `POST /v1/messages` 를 `localhost:8000` 으로 → LiteLLM 의 `anthropic_proxy` (litellm-config.yaml 마지막 블록) 가 Anthropic Messages API 를 OpenAI chat API 로 변환 → `model_name=ollama/qwen3-coder-next:q8_0` 매칭 → ollama-lb → Ollama 노드.
...
| `ANTHROPIC_DEFAULT_SONNET_MODEL` | 균형형 | `ollama/qwen3-coder-next:q4_K_M` | `ollama/qwen3-coder-next:q8_0` |
| `ANTHROPIC_DEFAULT_HAIKU_MODEL` | 가벼운 작업 | `ollama/gemma4:26b` | `ollama/qwen3.5:9b` |
...
|---|---|---|---|
| `qwen3-coder-next:q4_K_M` | ~51 GB | ~50 t/s | iterative coding, 빠른 응답 |
| `qwen3-coder-next:q8_0` | ~84 GB | ~25 t/s | 어려운 리팩터, deep reasoning |

VRAM 부족하면 q4_K_M 을 메인으로. q8_0 을 슬롯 매핑에 두면 Ollama 가 자동으로 q4 → q8 모델 스왑하므로 컨텍스트 전환 latency (~30 s) 발생 — 한 작업에는 한 quant 만 쓰는 게 유리.
...
| `Ollama 노드 unreachable` | `setup.sh` 가 intersection discovery 시 warn 으로 끝남 — 모델 등록 안 됨. 노드에서 `ollama pull qwen3-coder-next:q8_0` |
