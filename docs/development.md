...
running [`kloudchat-backend`](https://github.com/boanlab/kloudchat-backend)
when you need to exercise a generation path.

## Web app

The production image serves a compiled bundle, so editing source has no effect
on it. Two ways to get hot reload:
...
```bash
docker compose logs -f kchat-api
docker compose restart kchat-api
docker compose exec kchat-api python -c "from app.core.config import settings; print(settings.env)"
```
...
# docker compose exec kchat-api alembic revision --autogenerate -m "…"
make migrate
```
...
**A built-in tool.** `app/services/tools/builtin.py` for the implementation and
`registry.py` for the schema. Tools are attached only to models that support
...
**Uploads fail with "permission denied" on a first boot.** The `kchat-init`
container exists to fix exactly this — Docker creates bind-mount directories as
root and the API runs as uid 1000. Check that `kchat-init` completed:
`docker compose ps -a | grep init`.

**Every `/api/` call 502s after recreating `kchat-api`.** The web container's
nginx resolves the upstream through a variable specifically to avoid this, so
if it happens, check that `KCHAT_API_URL` is set on `kchat-web`.
