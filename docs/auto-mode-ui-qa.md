...
- 개인정보 처리로 분류기를 생략하는 API 경로도 원래 Auto 모드를 기록함. 마스킹 정책과 분류기 생략 순서는 변경하지 않음.
...
- API의 모드 보존은 enum·저장 문자열 각각의 절약·품질 조합 4건으로 검증함. 수정 전 품질 2건 실패·절약 2건 통과에서 수정 후 4건 통과함. 합성 응답을 사용하되 실제 요청 처리·SSE·메시지 저장·감사 기록 경로를 확인하고 분류기 호출 0회를 검증함.
