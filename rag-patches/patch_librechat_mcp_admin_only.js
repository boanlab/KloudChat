...
// 비-ADMIN 사용자가 본인 agent 에 부착·사용 못하게 차단. 관리자가 만든 공유 agent
// 실행에는 영향 없음(아래는 agent 생성/수정/복제 시점 도구 인가 + 빌더 MCP 목록만 수정).
//
//  (1) controllers/agents/v1.js : filterAuthorizedTools 가 create/update/duplicate 모두에서
//      도구 인가 → userRole 받아 비-admin 의 제한 MCP 서버 도구 거부(서버측 강제).
//  (2) controllers/mcp.js : getMCPTools 가 빌더의 'MCP 도구 추가' 목록 생성 — 비-admin 에겐
...
patch(V1, [
  ...
  // MCP 도구 파싱 후, 비-admin 의 제한 서버 도구 거부 (existing/registry 보존보다 먼저).
  [
