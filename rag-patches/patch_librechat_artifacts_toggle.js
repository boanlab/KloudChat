// 저장된(DB) 에이전트도 채팅의 Artifacts 토글(배지)을 따르게 한다.
//
// 원본 동작
// ---------
// LibreChat 의 artifacts 시스템 프롬프트는 @librechat/api 의 initializeAgentOptions
// (packages/api/dist/index.js) 에서 `agent.artifacts` 가 빈 문자열이 아니면 무조건 주입된다.
//   - 커스텀 endpoint(ephemeral) 대화: loadEphemeralAgent 가 req.body.ephemeralAgent.artifacts
//     (= 채팅 Artifacts 배지 토글 값) 를 agent.artifacts 로 채움 → 토글이 그대로 먹힌다.
//   - 저장된 DB 에이전트(Super Agent/모델 에이전트) 대화: agent.artifacts 가 DB 저장값 그대로라
//     배지 토글을 무시한다. (agents endpoint 의 buildOptions 가 override 를 안 함.)
//
// 패치
// ----
// agents/build.js 의 buildOptions 가 반환하는 agent(Promise)를 후처리해서, 요청에
// ephemeralAgent.artifacts(문자열)가 있으면 그 값으로 agent.artifacts 를 덮어쓴다.
// 이 agent 객체는 initialize.js 가 await 후 그대로 initializeAgent 에 넘기므로(같은 참조),
// 이후 프롬프트 게이팅이 override 된 값을 읽는다. 프롬프트 생성 자체는 기존 로직이 담당.
//
//   배지 ON          → agent.artifacts = 'default' (override)  → 내장 프롬프트 주입
//   배지 OFF/미전송   → agent 저장값(DB) 유지
//
// manage.sh 는 image/ppt/video 를 artifacts:'default'(기본 ON), 그 외는 ''(off)로 sync.
// 따라서 Image/Slide/Video Studio 는 기본 ON, 나머지는 토글로 ON. (배지가 force-on 만
...
// artifacts ON 일 때 모델이 :::artifact 를 즉시 출력하지 않고 execute_code/web_search/
// fetch 로 빌드·실행·반복하다 tool 루프에 빠지는 걸 막는 anti-loop 가이드. artifact 은
// 프롬프트가 아니라 토글로만 트리거되므로, 이 가이드도 토글이 켜진 요청에만 주입한다.
// agent.instructions 에 append (initializeAgent 는 instructions 를 덮어쓰지 않고
