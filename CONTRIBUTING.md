...
kchat is two containers and a database. Models and tools are **not** part of
this repository — they live in [`kloudchat-backend`][backend], and kchat
...
The production image serves a static bundle, so source edits do not appear.
Overlay the dev compose file to swap in Vite with hot reload:
...
```bash
docker compose exec kchat-api alembic revision --autogenerate -m "short description"
docker compose exec kchat-api alembic upgrade head
```
