...
- `./scripts/monitor.sh` — 컨테이너 + systemd 로그 라이브 스트림 (LibreChat / LiteLLM / Ollama / ComfyUI / Whisper 통합)
- `./scripts/gpu-monitor.py` — GPU 노드 라이브 TUI 대시보드 (노드별 VRAM / 로드 모델 / 큐). 첫 실행 시 `uv` 가 rich + httpx 자동 설치
