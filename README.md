...
- `./scripts/monitor.py` — 통합 라이브 TUI: 상단 GPU 노드 대시보드 (VRAM torch+cache 분리 / 로드 모델 / 큐 / LiteLLM 카탈로그) + 하단 컨테이너+systemd 로그 스트림. `--dashboard` / `--logs` / `--once` / `--panels` 옵션. 첫 실행 시 `uv` 가 rich + httpx 자동 설치
