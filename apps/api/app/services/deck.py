...
which would put invented numbers into a .pptx; `image` has no producer behind
it; `two-column` renders identically to `bullets`. The model is offered only
...
#: A talk, not a document — and each slide is its own model call.
_MIN_SLIDES = 5
_MAX_SLIDES = 12

#: The only layouts with a renderer behind them. See the module docstring.
_LAYOUTS = ("title", "bullets", "quote")

#: One accent for the whole deck, applied to every slide. The preview and the
#: .pptx read from the same field, so they cannot drift apart.
_ACCENT = "#5b5bd6"
...
규칙:
- bullets 는 3~5개. 각 항목은 한 줄, 40자 이내. 문장 부호로 끝내지 마라.
    ...
    *,
    ...
    try:
        text, spent = await _complete(
            model,
            _OUTLINE_PROMPT.format(lo=_MIN_SLIDES, hi=_MAX_SLIDES, request=request[:2000]),
            api_key,
            600,
        )
    ...
    # Announced up front so the panel can show the whole deck and grey out what
    ...
    for index, slide in enumerate(slides):
        label = f"{index + 1}/{len(slides)} {slide['title']}"
        ...
        if slide["layout"] == "title":
            ...
            slide["notes"] = ""
            yield {"type": "step", "id": slide["id"], "label": label, "status": "done"}
            yield {"type": "slide", "slide": slide, "done": True}
            continue

        yield {"type": "step", "id": slide["id"], "label": label, "status": "running"}
        ...
        try:
            body, spent = await _complete(
                ...
                template.format(
                    ...
                    # Only the tail: the whole deck would crowd out the
                    # instruction by the time slide ten is written.
            ...
            yield {"type": "step", "id": slide["id"], "label": label, "status": "error"}
