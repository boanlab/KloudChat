...
async def check_ncs_answer(arguments: dict[str, Any]) -> ToolResult:
    ...
    if decision == "calculate":
        if set(arguments) - _CALCULATE_FIELDS:
            return _invalid()
        result = await arithmetic.calculate(
            {name: arguments[name] for name in _ARITHMETIC_FIELDS if name in arguments}
        )
        data = {
...
CHECK_NCS_ANSWER = Tool(
    ...
    parameters={
        ...
        "properties": {
            ...
            **deepcopy(arithmetic.CALCULATE.parameters["properties"]),
            "reason": {
                "type": "string",
                "minLength": 1,
