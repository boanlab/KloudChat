...
async def test_settings(admin: AdminUser):
    ...
    try:
        ...
        return {"ok": False, "detail": f"연결은 되지만 모델 목록을 읽지 못했습니다 ({exc})."}
...
async def test_smtp(payload: SmtpTestRequest, admin: AdminUser):
    ...
    try:
        await mail_service.send(
            to=recipient,
            subject="kchat 메일 설정 테스트",
            body=(
                "이 메일이 도착했다면 kchat 의 메일 설정이 올바릅니다.\n"
...
async def suspend(user_id: str, request: Request, admin: AdminUser, db: DbSession):
    ...
    # step with kchat's, which is the whole point of per-user keys.
...
async def set_credits(
    ...
    # kchat owns the limit and the proxy mirrors it; unsynced, the backstop
