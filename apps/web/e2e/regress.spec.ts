/** Regressions from real use, against a real backend and model. */
...
test('웹 검색을 켜도 화면이 살아 있다 (스텝 렌더 크래시)', async ({ page }) => {
  ...
  // The prompt must require a lookup: the toggle offers the tool, it does not force a call.
  ...
  // The URL lands as soon as the session exists, not when the answer is done.
  await expect(page).toHaveURL(/\/s\/[0-9a-f]{32}/, { timeout: 10_000 })

  // Step labels survive completion, so wait for the turn to finish first.
  await expect(page.getByLabel('중지')).toHaveCount(0, { timeout: 300_000 })

  // An unknown step `type` must not unmount the tree. A finished search collapses to a summary.
  await expect(page.getByText(/웹 검색( 중)?/).first()).toBeVisible({ timeout: 20_000 })
  await expect(page.getByLabel('프롬프트 입력')).toBeVisible()
  // The shell is the tree a step-render crash would take with it.
...
test('대화 안에서 모델을 바꾸면 그 대화에 반영된다', async ({ page }) => {
  ...
  // Until the row lands in the store the picker has no conversation to write to.
  ...
  await picker.click()
  // Entries are plain <button> inside role="menu" (components/ui Dropdown), not menuitem.
  const menu = page.getByRole('menu')
  await expect(menu).toBeVisible({ timeout: 10_000 })
  const other = menu
    .getByRole('button')
    .filter({ hasNotText: /Qwen3\.[56]/i })
    // Picker rows price as '{n} 크레딧'; '1k당 입력' is the settings screen's wording.
    .filter({ hasText: /크레딧/ })
    .first()
...
test('사용자마다 자기 LiteLLM 키가 있고, 관리자가 회전시킬 수 있다', async ({ page }) => {
  // "전용 키 없음" means spend attribution is broken.
  await page.goto('/admin/users')
  // The table pages at forty rows; a bare /검색/ also matches the sidebar search.
  await page.getByPlaceholder('이름 또는 이메일').fill(E2E_ADMIN.email)

  const row = page.locator('tr', { hasText: E2E_ADMIN.email })
  await expect(row).toBeVisible({ timeout: 15_000 })

  // An account without a key shows "전용 키 없음"; the button handles both cases.
  ...
  // Rotation is three round trips to the proxy.
...
test('크레딧 한도를 바꾸면 LiteLLM 에 반영될 금액이 함께 보인다', async ({ page }) => {
  // The quoted amount is served by the API.
  await page.goto('/admin/users')
  await page.getByPlaceholder('이름 또는 이메일').fill(E2E_ADMIN.email)
  const row = page.locator('tr', { hasText: E2E_ADMIN.email })
  await expect(row).toBeVisible({ timeout: 15_000 })
...
test('컴포저 메뉴가 화면 밖으로 나가지 않는다', async ({ page }) => {
  // Composer menus are anchored near the bottom of the window.
  await page.goto('/new/chat')
  ...
  for (const [label, open] of opens) {
    // The connector menu exists only once the account has a usable connector.
    if (
      label === '커넥터' &&
...
test('환경설정 스위치가 계정에 저장되고 화면에 반영된다', async ({ page }) => {
  ...
  // Account-wide setting: an earlier run may have left it either way.
  ...
  // The switch writes in the background; wait for the PATCH before reloading.
  await Promise.all([
    page.waitForResponse(
      (r) => r.url().endsWith('/api/auth/me') && r.request().method() === 'PATCH',
      { timeout: 30_000 },
  ...
  try {
    ...
    // Restore: later tests read this switch.
    await page.goto('/settings/preferences')
    await page
...
test('보고서를 만들면 섹션이 채워지고 내보낼 수 있다', async ({ page }) => {
  ...
  // The outline lands first, so the 목차 button (보기 tab) has a denominator; then the count completes.
...
test('그림·클립 화면에는 보낼 곳 없는 첨부·스킬 버튼이 없다', async ({ page }) => {
  // The image/av endpoints take a prompt and option chips only; an attachment or skill would be dropped.
  await page.goto('/new/chat')
  ...
  for (const surface of ['image', 'av'] as const) {
    ...
    // A hidden file input is still reachable.
...
test('한국어 굵은 글씨가 별표로 새지 않는다', async ({ page }) => {
  // CommonMark will not close `**` between a bracket and a Korean particle.
  await page.goto('/new/chat')
  await pickLocal(page)
