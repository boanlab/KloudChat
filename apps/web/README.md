...
The single-page app. It talks to exactly one thing — the kchat API — and has no
...
repository root, `docker compose -f docker-compose.yml -f docker-compose.dev.yml
up -d` does the same thing inside Docker.
