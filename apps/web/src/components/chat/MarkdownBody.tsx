...
export function MarkdownBody({
  ...
  return (
    <div
      className={cn(
        "text-md leading-[1.7] break-words max-sm:text-[1.0625rem] max-sm:leading-[1.75]",
        className,
      )}
    >
      <ReactMarkdown
        ...
        components={{
          ...
          ul: ({ children }) => (
            <ul className="my-2.5 list-disc space-y-1 pl-5 marker:text-faint">
              {children}
            </ul>
          ),
          ...
          ol: ({ children, start }) => (
            <ol
              start={start}
              className="my-2.5 list-decimal space-y-1 pl-5 marker:text-faint"
            >
              {children}
            </ol>
          ),
          ...
          th: ({ children }) => (
            <th className="border-b border-line px-3 py-2 text-left font-semibold break-keep">
              {children}
            </th>
          ),
          ...
          code: ({ className, children, ...props }) => {
            ...
            if (/language-chart/.test(className ?? "")) {
              return (
                <ChartBlock source={String(children).trimEnd()} owner={owner} />
              );
            }
          ...
          img: ({ src, alt }) => (
            <span className="my-3 block">
              ...
              />
              {alt ? (
                <span className="mt-1.5 block text-base text-muted">{alt}</span>
              ) : null}
            </span>
          ),
