...
Single tool `transcript(url, language=None)`:
  1. youtube-transcript-api 로 자막 시도 (영상에 자막 있으면 GPU 미사용 — 즉시 반환).
  ...
  3. WHISPER_URL(보통 compose 내부 whisper-shim http://whisper-shim:9000)로 전사.
     ...
     whisper 는 GPU 전용 — OR STT 폴백은 없다. 미설정/미가용이면 자막 있는 영상만 동작.
     (애초에 WHISPER_URLS 가 비면 이 도구 자체가 에이전트에 부착되지 않는다 — manage.sh 게이팅.)
...
async def _whisper(audio_path: str) -> str:
    """audio 를 WHISPER_URL(whisper-shim → GPU backend)로 전사. GPU 전용 — OR 폴백 없음."""
    if not WHISPER_URL:
...
async def transcript(url: str, language: str | None = None) -> str:
    """YouTube 영상의 자막/스크립트/전사 텍스트를 반환한다.

    한국어 트리거: "유튜브 자막", "유튜브 스크립트", "영상 내용 알려줘",
    "유튜브 요약해줘"(먼저 transcript 를 받아 그 본문을 모델이 요약), "영상 텍스트",
    "YouTube 내용", "이 영상 무슨 내용", URL/링크가 youtube.com / youtu.be 인 경우.
    Returns the full transcript text of a YouTube video.

    자막이 있으면 그대로 반환하고, 없으면 audio 를 받아 whisper(로컬 GPU)로 전사한다.
    whisper 미가용이면 전사 불가(자막 있는 영상만 동작). 짧은 영상은 수 초, 긴 영상은
    분 단위 소요된다.
