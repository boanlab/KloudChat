...
relevance evaluation 을 적층 → 회수율/정밀도 향상. 부품 전부 로컬:
...
RERANK_URL 이 비면 rerank 단계만 무변형 통과(reranker 미배포 환경 = hybrid 까지).

테넌시 & 역할 분리
----------------
모든 쿼리에 `cmetadata->>'user_id' = LIBRECHAT_USER_ID` 강제. LibreChat 이
`{{LIBRECHAT_USER_ID}}` 를 호출자별로 치환(per-session stdio spawn, startup:false),
USER_ID 가 비면 전역 검색 대신 **거부**(usage.py 동일 규율) — 타 사용자
업로드 문서 누출 방지.

**범위 = 사용자 본인 업로드 전용.** 에이전트 지식(`cmetadata.user_id`=agent id)은
native `file_search`(LibreChat 가 agent 의 file_ids 를 서버측 주입해 스코프) 담당.
smart_search 가 에이전트 지식 미처리 이유 = LibreChat MCP placeholder 가 agent id 를
미노출(body placeholder 화이트리스트=conversationId/parentMessageId/
messageId). 자세한 근거는 docs/user/smart-search.md.
"""
...
# eval 단계 = gemma 1콜 추가 — latency 민감 시 false 로 차단하면 dense+reformulate 만.
...
# sparse 는 plainto_tsquery 의 AND(&)를 OR(|)로 치환 → 다어절 서브쿼리 recall
# 확보(한 청크에 모든 토큰 없어도 매칭 → ts_rank 정렬). dense=의미,
# sparse=정확 키워드 담당. FTS 인덱스(ix_lpe_fts)는 _ensure_fts_index() 가 보장.
_HYBRID_SQL = """
WITH dense AS (
...
def _ensure_fts_index() -> None:
    ...
    없어도 seq-scan 으로 동작하나 느림 → 프로세스 기동 시 1회 생성. stdio
    MCP 라 stdout 출력 금지 — 실패는 stderr 로만, 검색은 계속."""
    if not DB:
        return
    try:
        with psycopg.connect(DB) as conn:
            ...
            conn.commit()
    except Exception as e:  # noqa: BLE001 — best-effort, 검색 기능은 인덱스 없이도 동작 가능
...
async def _retrieve(subqueries: list[str], k: int = 20) -> list[dict]:
    """각 서브쿼리 hybrid 검색 → RRF 로 multi-query 병합."""
    ...
    with psycopg.connect(DB) as conn:
        for q, v in zip(subqueries, vecs):
            ...
            for document, meta, rrf in rows:
                ...
                row["score"] += float(rrf)  # within-subquery RRF → multi-query 합산
    return sorted(merged.values(), key=lambda r: -r["score"])[:k]


async def _rerank(query: str, cands: list[dict], n: int = 8) -> list[dict]:
    """bge-reranker-v2-m3 cross-encoder 로 후보 재정렬.

    RERANK 미설정 시 **무변형 통과**(Phase 1 동작 유지). hybrid 가 RRF 로 모은
    후보를 교차 인코더가 (질문, 청크) 쌍 직접 채점 → 정밀 재정렬.
    엔드포인트 = vLLM `--runner pooling`(/rerank, Jina 호환) 또는 동등 서버.
...
async def _evaluate(query: str, cands: list[dict]) -> list[dict]:
    """gemma 1콜로 후보 relevance 일괄 판정 → 노이즈 컷."""
    if not cands:
        return cands
...
async def smart_search(query: str, top_k: int = 8) -> list[dict]:
    """업로드 문서에서 질문에 답할 근거를 정밀 검색.

    reformulation(다중 쿼리) → dense 검색(RRF 병합) → relevance 평가 순 —
    단순 키워드 매칭이 놓치는 동의어·우회 표현까지 회수. 인용 가능한 발췌
    청크 리스트 반환(각 항목에 file_id/source 포함).
